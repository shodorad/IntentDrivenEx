...
|------|-------------|-----------|-----------|----------|
| FIX-001 | Typo `[/ACTION_PUPILS]` → `[/ACTION_PILLS]` | [ ] Pending | — | 🔴 Critical |
| FIX-002 | ES pill labels — pills stay English on toggle | [ ] Pending | — | 🔴 High |
| FIX-003 | ES SignalBanner CTA stays English | [ ] Pending | — | 🔴 High |
| FIX-004 | Derek flowKey routes to new-phone instead of upgrade | [ ] Pending | — | 🔴 High |
| FIX-005 | Robert UpgradeFlow — wrong prices, 2-plan only, no line toggle | [ ] Pending | — | 🔴 High |
| FIX-006 | RefillFlow hardcoded Visa ····4821 | [ ] Pending | — | 🟡 Medium |
| FIX-007 | UpgradeFlow hardcoded plan names | [ ] Pending | — | 🟡 Medium |
| FIX-008 | SMS modal same message for all transaction types | [ ] Pending | — | 🟡 Medium |
| FIX-009 | InternationalFlow "Mexico" hardcoded — should be persona country | [ ] Pending | — | 🟡 Medium |
| FIX-010 | Carlos flowKey routing misses renewal pill labels | [ ] Pending | — | 🟡 Medium |
| FIX-011 | Persona dropdown native `<select>` — cosmetic | [ ] Pending | — | 🟢 Low |
| FIX-012 | James activation — text-only, no inline card | [ ] Pending | — | 🟢 Low |
| FIX-013 | Angela Live Chat hardcoded "Available now" | [ ] Pending | — | 🟢 Low |

**Demo readiness:**
- [ ] Ready for Lam review
- [ ] Ready for Srini review
- [x] Needs fixes — see Critical/High items above
...
Priority 1 — Critical blockers (fix before any review)
  [ ] FIX-001  Typo: [/ACTION_PUPILS] → [/ACTION_PILLS] in demoResponses.js
...
Priority 2 — High (fix before Lam review)
  ...
  [ ] FIX-009  InternationalFlow: replace "Mexico" with persona.account.internationalCallsThisMonth[0].country
...
Priority 4 — Low / polish (if time allows)
  ...
  [ ] FIX-013  Angela: Live Chat availability respects time of day
```
...
**Known gaps / QA focus:**
Confirm the fix does not affect any other branches (search for the exact string `[/ACTION_PUPILS]` — should be exactly 1 occurrence, at line ~209 only).
...
**Known gaps / QA focus:**
Confirm all 8 personas' suggestedActions have `labelEs` entries — no undefined/null values for any pill in ES mode.
...
**Known gaps / QA focus:**
Check that `banner.flowId` is not `undefined` for any persona — all 8 personas' `intentCategory` values must be valid keys in `CTA_KEYS`.
...
```
[ ] C4 — SignalBanner CTA "Recarga Rápida" in ES mode (FIX-003)
[ ] C5 — Pills in Spanish: "Mi internet está lento", "Me quedo sin datos", "Quiero gastar menos" (FIX-002)
[ ] E5 — Maria plan-change branch shows quick reply pills (FIX-001)
[ ] F2 — Refill card shows correct persona savedCard and plan name (FIX-006)
[ ] F2 — Upgrade card shows correct persona current plan (FIX-007)
[ ] F5 — SMS modal refill message: "5 GB added. Valid through [date]." (FIX-008)
[ ] F5 — SMS modal upgrade message: references upgraded plan name (FIX-008)
[ ] F5 — SMS modal international message: references correct country (FIX-008, FIX-009)
```

### Persona demo path re-verification
```
[ ] us-001 Maria: refill path end-to-end ✓ | plan-change branch pills ✓ (FIX-001)
[ ] us-002 Carlos: turn 2 routes to renewal options in demo mode (FIX-010)
[ ] us-003 Priya: no regression from Sprint 1 pass ✓
[ ] us-004 James: empty dashboard no regression ✓ | ActivationFlow card if FIX-012 done
[ ] us-005 Angela: no regression ✓ | after-hours check if FIX-013 done
[ ] us-006 Derek: turn 2 shows upgrade options (NOT phone questions) (FIX-004)
[ ] us-006 Derek: UpgradeFlow shows "Total Base 5G" as current plan (FIX-007)
[ ] us-007 Ana: InternationalFlow shows "Colombia" not "Mexico" (FIX-009)
[ ] us-008 Robert: FamilyPlanCompare shows 3 plans + line toggle + correct 4-line prices (FIX-005)
```

### Language toggle full re-verification (ES mode)
```
[ ] C1 — ES pill active state (teal) — no regression
[ ] C2 — Headline in Spanish — no regression
[ ] C3 — Subhead "Soy ClearPath AI" — no regression
[ ] C4 — SignalBanner CTA: "Recarga Rápida" ✓ (FIX-003)
[ ] C5 — Pills in Spanish ✓ (FIX-002)
[ ] C6 — Trust banner "Cómo funciona" — no regression
[ ] C7 — Input placeholder "Escribe tu mensaje..." — no regression
```

### Regression checks (Sprint 1 passes that must not break)
```
[ ] TASK-001 — Persona data still imports correctly
[ ] TASK-003 — URL routing still works (?persona=maria, ?persona=us-006, etc.)
[ ] TASK-004 — MiniDashboard still reads from persona.account
[ ] TASK-005 — 3 signals per persona still render
[ ] TASK-006 — Data meter color thresholds unchanged
[ ] TASK-011 — Priya rewards path: free option surfaced first
[ ] TASK-017 — Clarifying question still fires before any recommendation (E4)
[ ] TASK-018 — 1.8s processing animation still enforced (F3)
[ ] TASK-020 — URL routing fallback still defaults to Maria (us-001)
```

...
function UsageTrend({ data = [], dataTotal }) {
  ...
  const capHits  = data.filter(d => Number(d.used || 0) >= max * 0.9).length;
  ...
  const insight = !data.length ? null
    : capHits >= data.length - 1
      ...
      : data.length >= 2 && data[data.length - 1].used > data[data.length - 2].used
        ...
        : `Avg ${(data.reduce((s, d) => s + Number(d.used || 0), 0) / data.length).toFixed(1)} GB/mo`;
...
export default function MiniDashboard({ onAddOnClick }) {
  ...
  // Return loading state if persona not yet loaded from BigQuery
  if (!account || !persona) {
    return (
      <div style={{ padding: '16px', textAlign: 'center', color: '#6B7280', fontSize: '0.85rem' }}>
        Loading account…
      </div>
    );
  }
  ...
  const dataRemaining = account.dataRemaining ? String(account.dataRemaining) : null;
  const dataTotal = account.dataTotal ? String(account.dataTotal) : null;

  const renewalDate = account.renewalDate ? String(account.renewalDate) : null;
